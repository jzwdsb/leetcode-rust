...
pub mod array_solution {
    ...
    fn test_array_sign() {
        ...
        assert_eq!(
            array_sign(vec![i32::MAX / 2, i32::MAX / 2, i32::MIN]),
            -1
        );
    }
    ...
    */
    ...
    fn test_can_jump() {
        ...
        assert_eq!(can_jump(vec![1, 1, 2, 2, 0, 1, 1]), true);
        assert_eq!(can_jump(vec![1, 2, 0, 1]), true);
    }
    ...
    fn test_length_of_last_word() {
        assert_eq!(
            length_of_last_word("Hello World".to_string()),
            5
        );
    ...
    fn test_insert_interval() {
        ...
        assert_eq!(
            insert_interval(vec![vec![1, 5]], vec![2, 3]),
            vec![vec![1, 5]]
        );
    }
    ...
    fn test_majority_element() {
        ...
        assert_eq!(
            majority_element(vec![2, 2, 1, 1, 1, 2, 2]),
            2
        );
    }
    ...
    fn test_longest_consecutive_sequence() {
        assert_eq!(
            longest_consecutive(vec![100, 4, 200, 1, 3, 2]),
            4
        );
        assert_eq!(
    ...
    fn test_can_place_flowers() {
        assert_eq!(
            can_place_flowers(vec![1, 0, 0, 0, 1], 1),
            true
        );
        assert_eq!(
            can_place_flowers(vec![1, 0, 0, 0, 1], 2),
            false
        );
        assert_eq!(
            can_place_flowers(vec![1, 0, 0, 0, 0, 1], 2),
            false
        );
        assert_eq!(
        ...
        assert_eq!(
            can_place_flowers(vec![0, 0, 1, 0, 1], 2),
            false
        );
    }
    ...
    fn test_erase_overlap_intervals() {
        assert_eq!(
            erase_overlap_intervals(vec![vec![1, 2], vec![2, 3], vec![3, 4]]),
            0
        );
        assert_eq!(
    ...
    fn test_asteroid_collision() {
        assert_eq!(
            asteroid_collision(vec![5, 10, -5]),
            vec![5, 10]
        );
        assert_eq!(
            asteroid_collision(vec![8, -8]),
            Vec::<i32>::new()
        );
        assert_eq!(
            asteroid_collision(vec![10, 2, -5]),
            vec![10]
        );
        assert_eq!(
            asteroid_collision(vec![-2, -1, 1, 2]),
            vec![]
        );
    }
    ...
    fn test_max_product() {
        ...
        assert_eq!(max_product(vec![-4, -3, -2]), 12)
    }
    ...
    pub fn test_single_number_ii() {
        ...
        assert_eq!(
            single_number_ii(vec![0, 1, 0, 1, 0, 1, 99]),
            99
        );
    }
    ...
    fn test_trap_rain() {
        assert_eq!(
            trap_rain(vec![0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2]),
            6
        );
    ...
    #[test]
    fn test_max_profits() {
        assert_eq!(max_profits(vec![7, 1, 5, 3, 6, 4]), 5);
        assert_eq!(max_profits(vec![7, 6, 4, 3, 1]), 0);
    }
    ...
    fn test_max_profit_iii() {
        assert_eq!(
            max_profit_iii(vec![3, 3, 5, 0, 0, 3, 1, 4]),
            6
        );
    ...
    fn test_max_profit_with_cool_down() {
        assert_eq!(
            max_profit_with_cool_down(vec![1, 2, 3, 0, 2]),
            3
        );
    }
    ...
    fn test_max_profit_iv() {
        assert_eq!(max_profit_iv(2, vec![2, 4, 1]), 2);
        assert_eq!(max_profit_iv(2, vec![3, 2, 6, 5, 0, 3]), 7);
    }

    #[test]
    fn test_stone_game_ii() {
        assert_eq!(stone_game_vii(vec![5, 3, 1, 4, 2]), 6);
        assert_eq!(
            stone_game_vii(vec![7, 90, 5, 1, 100, 10, 10, 2]),
            122
        );
    }
    ...
    fn test_missing_positive() {
        ...
        assert_eq!(
            first_missing_positive(vec![7, 8, 9, 11, 12]),
            1
        );
    ...
    fn test_number_of_good_pairs() {
        assert_eq!(
            number_of_good_pairs(vec![1, 2, 3, 1, 1, 3]),
            4
        );
        assert_eq!(number_of_good_pairs(vec![1, 1, 1, 1]), 6);
        assert_eq!(number_of_good_pairs(vec![1, 2, 3]), 0);
    }
    ...
    #[test]
    fn test_find_matrix() {
        let mut res = find_matrix(vec![1, 3, 4, 1, 2, 3, 1]);
    ...
    fn test_largest_number() {
        ...
        assert_eq!(largest_number(vec![10]), "10");
    ...
    fn test_divide_array() {
        assert_eq!(
            divide_array(vec![1, 3, 4, 8, 7, 9, 3, 5, 1], 23),
    ...
    fn test_group_the_people() {
        let mut res = group_the_people(vec![3, 3, 3, 3, 3, 1, 3]);
        for row in res.iter_mut() {
            row.sort();
        }
        res.sort();
        assert_eq!(res, vec![vec![0, 1, 2], vec![3, 4, 6], vec![5]]);
        let mut res = group_the_people(vec![2, 1, 3, 3, 3, 2]);
        for row in res.iter_mut() {
            row.sort();
        }
        res.sort();
    ...
    fn test_zigzag_convert() {
        assert_eq!(
            zigzag_convert("PAYPALISHIRING".to_string(), 3),
            "PAHNAPLSIIGYIR"
        );
        assert_eq!(
            zigzag_convert("PAYPALISHIRING".to_string(), 4),
            "PINALSIGYAHRPI"
        );
        assert_eq!(zigzag_convert("A".to_string(), 1), "A");
    }
    ...
    fn test_largest_perimeter() {
        ...
        assert_eq!(
            largest_perimeter(vec![1, 12, 1, 2, 5, 50, 3]),
            12
        );
    ...
    fn test_find_least_num_of_unique_ints() {
        ...
        assert_eq!(
            find_least_num_of_unique_ints(vec![5, 5, 4], 1),
            1
        );
        assert_eq!(
            find_least_num_of_unique_ints(vec![2, 1, 1, 3, 3, 3], 3),
            1
        );
        assert_eq!(find_least_num_of_unique_ints(vec![1], 1), 0,)
    }

    #[test]
    fn test_furthest_building() {
        assert_eq!(
            furthest_building(vec![4, 2, 7, 6, 9, 14, 12], 5, 1),
            4
        );
        assert_eq!(
        ...
        assert_eq!(
            furthest_building(vec![14, 3, 19, 3], 17, 0),
            3
        );
    }
    ...
    fn test_most_booked_iii() {
        assert_eq!(
            most_booked_iii(2, vec![(0, 10), (1, 5), (2, 7), (3, 4)]),
            0
        );
        assert_eq!(
            most_booked_iii(3, vec![(1, 20), (2, 10), (3, 5), (4, 9), (6, 8)]),
            1
        );
    }
    ...
    fn test_find_judge() {
        ...
        assert_eq!(
            find_judge(3, vec![vec![1, 3], vec![2, 3]]),
            3
        );
        assert_eq!(
    ...
    fn test_bag_of_tokens_score() {
        ...
        assert_eq!(
            bag_of_tokens_score(vec![100, 200, 300, 400], 200),
            2
        );
    }
    ...
    fn test_min_operations() {
        ...
        assert_eq!(min_operations(vec![1, 10, 100, 1000]), 3);
    }

    #[test]
    fn test_max_frequency_elements() {
        assert_eq!(
            max_frequency_elements(vec![1, 2, 2, 3, 1, 4]),
            4
        );
        assert_eq!(
            max_frequency_elements(vec![1, 2, 3, 4, 5]),
            5
        );
    }
    ...
    fn test_array_intersections() {
        let mut res = intersection(vec![1, 2, 2, 1], vec![2, 2]);
        res.sort();
        assert_eq!(res, vec![2]);
        let mut res = intersection(vec![4, 9, 5], vec![9, 4, 9, 8, 4]);
        res.sort();
    ...
    fn test_num_subarray_with_sum() {
        assert_eq!(
            num_subarray_with_sum(vec![1, 0, 1, 0, 1], 2),
            4
        );
        assert_eq!(num_subarray_with_sum(vec![1, 1, 1], 2), 2);
    }
    ...
    fn test_number_of_boomerangs() {
        ...
        assert_eq!(
            number_of_boomerangs(vec![vec![1, 1], vec![2, 2], vec![3, 3]]),
            2
        );
